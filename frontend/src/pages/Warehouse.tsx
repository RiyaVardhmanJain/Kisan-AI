...
import type { WarehouseData, LotData, AlertData, StorageEventData, ConditionsData } from '../services/warehouseService';
...
const CROP_OPTIONS = [
    'Onion', 'Potato', 'Wheat', 'Rice', 'Tomato', 'Cotton',
    'Sugarcane', 'Garlic', 'Maize', 'Soybean', 'Groundnut',
    'Banana', 'Mango', 'Grapes', 'Pomegranate',
];

/* ─── Shelf life lookup (days) ─── */
const SHELF_LIFE_DAYS: Record<string, number> = {
    'Onion': 30, 'Potato': 60, 'Wheat': 180, 'Rice': 365,
    'Tomato': 7, 'Cotton': 365, 'Sugarcane': 14, 'Garlic': 60,
    'Maize': 180, 'Soybean': 180, 'Groundnut': 120,
    'Banana': 5, 'Mango': 10, 'Grapes': 7, 'Pomegranate': 30,
};

/* ─── Helpers (ported from StorageDashboard / DistributionPlanner) ─── */
function generateSensorData(warehouse: WarehouseData): StorageUnit {
    const baseTemp = warehouse.type === 'cold_storage' ? 4 : warehouse.type === 'ventilated' ? 22 : 28;
    const baseHumidity = warehouse.type === 'cold_storage' ? 85 : warehouse.type === 'ventilated' ? 55 : 65;
    ...
    return {
        ...
        produceStored: [],
        capacityUsed: warehouse.capacityQuintals > 0
            ? Math.round((warehouse.usedCapacity / warehouse.capacityQuintals) * 100)
            : 0,
    };
}
...
function getShelfLifePercent(lot: LotData): number {
    ...
    const shelfLife = lot.expectedShelfLifeDays || SHELF_LIFE_DAYS[lot.cropName] || 30;
    return Math.max(0, Math.min(100, Math.round(((shelfLife - daysStored) / shelfLife) * 100)));
}
...
function getConditionColor(condition: string): string {
    switch (condition) {
        ...
        case 'spoiled': return 'text-red-800 bg-red-100 border-red-300';
        default: return 'text-gray-600 bg-gray-50 border-gray-200';
    }
}

function extractPricesFromStateData(stateData: any, stateName: string, cropName: string): StatePrice[] {
...
const Warehouse: React.FC = () => {
    ...
    // Tab state
    const [activeTab, setActiveTab] = useState<'storage' | 'monitor' | 'distribution'>('storage');
    ...
    // Forms
    const [whForm, setWhForm] = useState({ name: '', city: '', type: 'dry', capacity: '' });
    const [lotForm, setLotForm] = useState({ warehouse: '', cropName: '', quantity: '', source: '' });

    // Monitor tab state
    const [sensorData, setSensorData] = useState<StorageUnit[]>([]);
    const [aiAnalysis, setAiAnalysis] = useState<StorageAnalysisResult | null>(null);
        ...
        try {
            ...
            const sensors = whData.map((w: WarehouseData) => {
                ...
                const warehouseLots = lotData.filter((l: LotData) => {
                    const wId = typeof l.warehouse === 'string' ? l.warehouse : l.warehouse?._id;
                    return wId === w._id;
                });
    ...
    const handleAddWarehouse = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const message = err?.response?.data?.error || err?.message || 'Failed to create warehouse';
    ...
    const handleAddLot = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const message = err?.response?.data?.error || err?.message || 'Failed to add lot';
    ...
    const handleDistributionAnalyze = async () => {
        ...
        try {
            ...
            const allPrices: StatePrice[] = [
                ...extractPricesFromStateData(mahaData, 'Maharashtra', cropName),
                ...extractPricesFromStateData(karnatakaData, 'Karnataka', cropName),
                ...extractPricesFromStateData(keralaData, 'Kerala', cropName),
                ...extractPricesFromStateData(punjabData, 'Punjab', cropName),
                ...extractPricesFromStateData(tamilnaduData, 'Tamil Nadu', cropName),
                ...extractPricesFromStateData(andraData, 'Andhra Pradesh', cropName),
                ...extractPricesFromStateData(teleganaData, 'Telangana', cropName),
            ];
    ...
    /* ─── Filtered lots ─── */
    const filteredLots = selectedWarehouse === 'all'
        ? lots
        : lots.filter((l) => {
            const whId = typeof l.warehouse === 'object' ? l.warehouse._id : l.warehouse;
            return whId === selectedWarehouse;
        });

    const storedLots = lots.filter((l: LotData) => l.status === 'stored' || l.status === 'partially_dispatched');
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-[#5B532C]/50 mt-1">
                                Welcome, {user?.name || 'Farmer'} · {warehouses.length} warehouse{warehouses.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                ...
                {loading ? (
                    <div className="flex items-center justify-center py-20">
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-[#5B532C]/50 text-sm">Loading your storage data...</p>
                        </div>
                    </div>
                ) : (
                    <>
                            ...
                            <>
                                ...
                                {warehouses.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        <h3 className="font-semibold text-[#5B532C]/60">No Warehouses Yet</h3>
                                        <p className="text-sm text-[#5B532C]/40 mt-1">Add your first warehouse to get started</p>
                                        <button
                                            onClick={() => setShowAddWarehouse(true)}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {warehouses.map((wh, i) => {
                                            ...
                                            const usedPct = wh.capacityQuintals > 0 ? (wh.usedCapacity / wh.capacityQuintals) * 100 : 0;
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="flex items-start justify-between mb-3">
                                                        <div>
                                                            <h3 className="font-semibold text-[#5B532C]">{wh.name}</h3>
                                                            <p className="text-xs text-[#5B532C]/50 flex items-center gap-1 mt-0.5">
                                                                <MapPin className="w-3 h-3" /> {wh.location.city}
                                                            </p>
                                                        </div>
                                                    ...
                                                    <div className="mb-3">
                                                        ...
                                                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                                            <motion.div
                                                                className={`h-full rounded-full ${usedPct > 90 ? 'bg-red-400' : usedPct > 70 ? 'bg-yellow-400' : 'bg-[#63A361]'
                                                                    }`}
                                                                initial={{ width: 0 }}
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-4">
                                        <h2 className="text-lg font-bold text-[#5B532C]">Your Stored Crops</h2>
                                    ...
                                    {filteredLots.length === 0 ? (
                                        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-[#5B532C]/15">
                                            ...
                                            <p className="text-sm text-[#5B532C]/40">No produce lots found</p>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-6">
                                ...
                                {aiAnalysis && (
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRiskBadge(aiAnalysis.overallRisk)}`}>
                                                {aiAnalysis.overallRisk.toUpperCase()} RISK
                                            </span>
                                        </div>
                                        <p className="text-sm text-[#5B532C]/80">{aiAnalysis.summary}</p>
                                    </motion.div>
                                )}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {sensorData.length === 0 ? (
                                        <div className="col-span-full text-center py-12">
                                            ...
                                            <p className="text-[#5B532C]/60">No storage units found. Add a warehouse first.</p>
                                        </div>
                                    ) : (
                                        sensorData.map((unit, idx) => {
                                            const unitAnalysis = aiAnalysis?.unitAnalyses.find(a => a.unitId === unit.unitId);
                                            return (
                                                <motion.div
                                                    key={unit.unitId}
                                                ...
                                                >
                                                    {/* Unit Header */}
                                                    <div className={`p-4 ${unit.type === 'cold_storage' ? 'bg-gradient-to-r from-blue-50 to-cyan-50' :
                                                        unit.type === 'ventilated' ? 'bg-gradient-to-r from-green-50 to-emerald-50' :
                                                            'bg-gradient-to-r from-amber-50 to-yellow-50'
                                                        }`}>
                                                        <div className="flex items-center justify-between">
                                                            <div>
                                                                <h3 className="font-bold text-[#5B532C]">{unit.name}</h3>
                                                                <p className="text-xs text-[#5B532C]/60 capitalize">{unit.type.replace('_', ' ')}</p>
                                                            </div>
                                                            {unitAnalysis && (
                                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskBadge(unitAnalysis.riskLevel)}`}>
                                                    ...
                                                    <div className="p-4">
                                                        <div className="flex justify-around mb-4">
                                                            <StorageGauge value={unit.temperature} max={50} label="Temp" unit="°C" color="#0ea5e9" />
                                                            <StorageGauge value={unit.humidity} max={100} label="Humidity" unit="%" color="#22c55e" />
                                                            <StorageGauge value={unit.capacityUsed} max={100} label="Capacity" unit="%" color="#f59e0b" />
                                                        </div>
                                                        ...
                                                        {(unit.temperature > 30 || unit.humidity > 75) && (
                                                            <div className="mb-3">
                                                                <SpoilageAlert
                                                                    severity={unit.temperature > 35 || unit.humidity > 85 ? 'critical' : 'warning'}
                                                                    title="Threshold Breach"
                                                                    message={`${unit.temperature > 30 ? `Temp: ${unit.temperature}°C` : ''} ${unit.humidity > 75 ? `Humidity: ${unit.humidity}%` : ''}`.trim()}
                                                                    recommendation={unit.temperature > 30 ? 'Activate cooling' : 'Activate dehumidifier'}
                                                                />
                                                            </div>
                                                        )}

                                                        {/* AI Recommendations */}
                                                        {unitAnalysis && unitAnalysis.recommendations.length > 0 && (
                                                            <div className="mt-3 p-3 bg-teal-50 rounded-lg border border-teal-100">
                                                                <p className="text-xs font-semibold text-teal-700 mb-1">AI Recommendations</p>
                                                                {unitAnalysis.recommendations.slice(0, 2).map((rec, i) => (
                                                                    <p key={i} className="text-xs text-teal-600">• {rec}</p>
                                                                ))}
                                                            </div>
                                                        )}
                                                        ...
                                                        {unit.produceStored.length > 0 && (
                                                            <div className="mt-3 flex flex-wrap gap-1">
                                                                {unit.produceStored.map((p, i) => (
                                                                    <span key={i} className="text-xs bg-[#FDE7B3]/40 text-[#5B532C] px-2 py-0.5 rounded-full">
                                                                        {p}
                                                                    </span>
                                                                ))}
                                                            </div>
                                ...
                                {lots.length > 0 && (
                                    <div className="space-y-4">
                                        <h2 className="text-lg font-bold text-[#5B532C]">Inventory Shelf Life</h2>
                                        {lots.map((lot, idx) => {
                                            const daysStored = getDaysStored(lot);
                                            const shelfLifePct = getShelfLifePercent(lot);
                                            const warehouseName = typeof lot.warehouse === 'string' ? lot.warehouse : lot.warehouse?.name || 'Unknown';
                                                    ...
                                                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                                                        <div className="flex-1">
                                                            <div className="flex items-center gap-3 mb-2">
                                                                <h4 className="font-bold text-[#5B532C] text-lg">{lot.cropName}</h4>
                                                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${getConditionColor(lot.currentCondition)}`}>
                                                                    {lot.currentCondition}
                                                                </span>
                                                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${lot.status === 'stored' ? 'bg-blue-50 text-blue-700' :
                                                                    lot.status === 'dispatched' ? 'bg-green-50 text-green-700' :
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <div className="text-center">
                                                                <p className="text-xs text-[#5B532C]/60 mb-1">Shelf Life</p>
                                                                <div className="w-24 bg-gray-200 rounded-full h-2.5">
                                                                    <div
                                                                        className={`h-2.5 rounded-full transition-all ${shelfLifePct > 50 ? 'bg-green-500' :
                                                                            shelfLifePct > 25 ? 'bg-amber-500' : 'bg-red-500'
                                                                            }`}
                                                                        style={{ width: `${shelfLifePct}%` }}
                                                                    />
                                                                </div>
                                                                <p className={`text-xs font-medium mt-1 ${shelfLifePct > 50 ? 'text-green-600' :
                                                                    shelfLifePct > 25 ? 'text-amber-600' : 'text-red-600'
                                                                    }`}>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="lg:col-span-1">
                                        <div className="bg-white rounded-xl shadow-lg border border-[#5B532C]/10 p-6">
                                            ...
                                            {storedLots.length === 0 ? (
                                                <p className="text-sm text-[#5B532C]/60">No stored lots available.</p>
                                            ) : (
                                                <div className="space-y-3">
                                                    {storedLots.map((lot) => {
                                                        const daysStored = getDaysStored(lot);
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <span className={`text-xs px-2 py-0.5 rounded-full ${daysStored > 14 ? 'bg-red-100 text-red-700' :
                                                                        daysStored > 7 ? 'bg-amber-100 text-amber-700' :
                                            ...
                                            {selectedDistLot && (
                                                <div className="mt-4">
                                                    <label className="block text-sm text-[#5B532C]/70 mb-1">Dispatch Quantity (quintals)</label>
                                                    <input
                                                        type="number"
                                                        step="0.1"
                                                        value={dispatchQty}
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full mt-3 px-4 py-2.5 bg-[#63A361] hover:bg-[#578f55] text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                                    >
                                                        {distAnalyzing ? (
                                                            <>
                                                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" />
                                        ...
                                        <AnimatePresence mode="wait">
                                            {!distResult && !distAnalyzing && (
                                                ...
                                                >
                                                    ...
                                                    <h3 className="text-lg font-semibold text-[#5B532C]/40">Select a lot and analyze</h3>
                                                    <p className="text-sm text-[#5B532C]/30 mt-1">AI will recommend the best markets for dispatch</p>
                                                </motion.div>
                                            )}
                                            ...
                                            {distAnalyzing && (
                                                ...
                                                >
                                                    ...
                                                    <p className="text-[#5B532C]/70">Analyzing markets across 7 states...</p>
                                                </motion.div>
                                            )}
                                            ...
                                            {distResult && !distAnalyzing && (
                                                ...
                                                >
                                                    {/* Urgency & Summary */}
                                                    <div className={`${urgencyConfig[distResult.urgency].bg} ${urgencyConfig[distResult.urgency].border} border rounded-xl p-5`}>
                                                        <div className="flex items-center justify-between mb-2">
                                                            <h3 className={`font-bold ${urgencyConfig[distResult.urgency].text} flex items-center gap-2`}>
                                                                {distResult.urgency === 'high' ? <AlertTriangle className="w-5 h-5" /> : <CheckCircle className="w-5 h-5" />}
                                                                Dispatch Recommendation
                                                            </h3>
                                                            <span className={`px-3 py-1 rounded-full text-sm font-medium ${urgencyConfig[distResult.urgency].badge}`}>
                                                                {distResult.urgency.toUpperCase()} URGENCY
                                                            </span>
                                                        </div>
                                                        <p className="text-sm text-[#5B532C]/80">{distResult.summary}</p>
                                                        <div className="mt-2 flex items-center gap-2 text-sm">
                                                            <ClockIcon className="w-4 h-4 text-[#5B532C]/50" />
                                                            <span className="text-[#5B532C]/70">{distResult.dispatchWindow}</span>
                                                        </div>
                                                    </div>
            ...
            <AnimatePresence>
                {showAddWarehouse && (
                    <>
                        ...
                        >
                            <div className="bg-white rounded-2xl shadow-xl border border-[#5B532C]/10 w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-between mb-5">
                                    <h3 className="text-lg font-bold text-[#5B532C]">Add Warehouse</h3>
                                    <button onClick={() => setShowAddWarehouse(false)} className="p-1 rounded-lg hover:bg-gray-100 text-[#5B532C]/40">
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                <form onSubmit={handleAddWarehouse} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Name</label>
                                        <input
                                            type="text"
                                            value={whForm.name}
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">City (for weather)</label>
                                        <input
                                            type="text"
                                            value={whForm.city}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Type</label>
                                            <select
                                                value={whForm.type}
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Capacity (qtl)</label>
                                            <input
                                                type="number"
                                                min="1"
                ...
                {showAddLot && (
                    <>
                        ...
                        >
                            <div className="bg-white rounded-2xl shadow-xl border border-[#5B532C]/10 w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-between mb-5">
                                    <h3 className="text-lg font-bold text-[#5B532C]">Add Produce Lot</h3>
                                    <button onClick={() => setShowAddLot(false)} className="p-1 rounded-lg hover:bg-gray-100 text-[#5B532C]/40">
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                <form onSubmit={handleAddLot} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Warehouse</label>
                                        <select
                                            value={lotForm.warehouse}
                                            onChange={(e) => setLotForm({ ...lotForm, warehouse: e.target.value })}
                                            className="w-full px-4 py-2.5 rounded-xl border border-[#5B532C]/15 text-[#5B532C] bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-[#63A361]/30"
                                        >
                                            <option value="">Select warehouse</option>
                                            {warehouses.map((wh) => (
                                                <option key={wh._id} value={wh._id}>{wh.name} — {wh.location.city}</option>
                                            ))}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Crop</label>
                                        <select
                                            value={lotForm.cropName}
                                            onChange={(e) => setLotForm({ ...lotForm, cropName: e.target.value })}
                                            className="w-full px-4 py-2.5 rounded-xl border border-[#5B532C]/15 text-[#5B532C] bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-[#63A361]/30"
                                        >
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Quantity (qtl)</label>
                                            <input
                                                type="number"
                                                min="0.1"
                                                step="0.1"
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-[#5B532C]/70 mb-1">Source (optional)</label>
                                            <input
                                                type="text"
                                                value={lotForm.source}
                                                onChange={(e) => setLotForm({ ...lotForm, source: e.target.value })}
